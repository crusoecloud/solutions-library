...
| 2026-07-24 | gcp | dual | >=1.9 | 0.5.46 | google 6.50.0 | 5.9.13-2ubuntu4 | 8.4.4 (ubuntu) | 1–4 | PASS | `ha_mode=dual`, 2 VMs across the tunnel split (vm0→tunnel-a, vm1→tunnel-b), GCP two-interface external gateway (TWO_IPS_REDUNDANCY). Each VM terminates its own tunnel (IKE ESTABLISHED) and learns the customer route via its own BGP session. **VM-failure test:** stopped vm1 mid-flight during a GCP→Crusoe ping — 150/180 received (16.7% loss over the 90 s window = BGP reconvergence gap ~15 s at 0.5 s interval, matching 10/30 timers + ECMP re-hash), then stable on the surviving vm0/tunnel-a. Connection survived. (First fresh pair earlier the same day was a transient Crusoe env glitch — never came up; a clean re-apply worked immediately.) |
