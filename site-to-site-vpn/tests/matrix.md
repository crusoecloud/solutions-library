...
| 2026-07-24 | gcp | dual | >=1.9 | 0.5.46 | google 6.50.0 | — | — | provision | BLOCKED (env) | `ha_mode=dual` applied cleanly: 2 VMs `STATE_RUNNING`, GCP repointed to both interfaces (TWO_IPS_REDUNDANCY), tunnel/SSH firewall rules verified correct (5 rules, minimal). But both VMs stayed non-functional for 25+ min — no SSH and both tunnels "No incoming packets from peer" (two IP-independent signals), unchanged by a VM reset. A Crusoe platform issue in eu-iceland1-a at the time (identical single-VM builds came up in ~5 min earlier the same day); module is validate-clean and differs from the proven single path only by VM count + tunnel-to-VM split. Live HA + VM-failure test pending a clean environment. |
