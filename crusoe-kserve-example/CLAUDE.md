...
**Autoscaling + gateway (experimental)**: three tiers — `make install-crusoe-autoscaler` (node tier — Crusoe cluster-autoscaler in `crusoe-system`, bounds `AUTOSCALER_MIN:MAX:POOL`, image must match the cluster's k8s minor) + `make install-gateway-scaling` (scales the Envoy gateway data plane 1×100m→3×2CPU + circuit breakers via `manifests/gateway-scaling.yaml` + a GatewayClass `parametersRef` patch) + `make install-vllm-hpa` (pod-tier metric pipeline — KEDA + a lightweight Prometheus on `vllm:num_requests_running`) + `make deploy-amd-mi355x AUTOSCALE=1` (emits `spec.scaling.wva.hpa` — KServe requires the `wva` mechanism when `scaling` is set; bare min/max is rejected). See `manifests/cluster-autoscaler.yaml`, `manifests/gateway-scaling.yaml`, `manifests/prometheus-vllm.yaml`, `manifests/keda-scaledobject.yaml`.

Load-testing findings (2× MI355X, 512/150): (1) default single Envoy proxy collapses to ~95% HTTP-500 at ~128 concurrent — `install-gateway-scaling` fixes it (0 fail through 256). (2) The single leader-elected EPP (`llm-d-inference-scheduler`) is the throughput ceiling: ~1.6k tok/s through the gateway vs ~9.7k direct (2 replicas), **independent of EPP CPU (3 vs 8) or scoring config** — architectural, not tunable. EPP is leader-elected single-active, so scale it *up* (3 CPU, in `amd.mi355x.scheduler`), not out (extra replicas idle at readiness `NOT_SERVING`). (3) Pod autoscaling is blocked three ways on a base CMK cluster: `num_requests_waiting` stays ~0 under gateway load (EPP gates admission); a standalone KEDA ScaledObject can't actuate (KServe owns `spec.replicas`, reverts external scaling in <10s); and native `spec.scaling.wva.hpa` (`AUTOSCALE=1`) needs a `VariantAutoscaling` CRD (`llmd.ai/v1alpha1`, from the llm-d workload-variant-autoscaler operator) AND a metrics-server — neither ships with base KServe/CMK, so the ISVC goes `Ready=False` (`ScalingCRDNotFound`) until installed. `wva.keda` on v0.19 has no custom-trigger field either. Node scale-up to a serving replica measured ~26 min (image pull + AITER compile dominate; node provision ~4.5 min).
...
- **Autoscaling gotcha (RWX shared disk):** when a new node is autoscaled in, its `fs.csi.crusoe.ai/*` node labels (which the RWX weights PV's `nodeAffinity` requires) land ~4.5 min *after* the node is `Ready` — the fs-node driver works immediately, only the labels lag. During that gap the new replica can't schedule (`PersistentVolume and node mismatch ... no matching NodeSelectorTerms`) **and the cluster-autoscaler over-provisions** (adds a 2nd node for 1 replica), later reclaiming it via scale-down. It self-resolves once the labels apply; to avoid it, pre-apply the labels or raise the autoscaler's scale-up delay. Full experiment write-up: the *MI355X Autoscaling Experiment* Confluence page.
