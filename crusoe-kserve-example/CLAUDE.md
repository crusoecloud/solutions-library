...
**Autoscaling (experimental)**: `make install-crusoe-autoscaler` (node tier — Crusoe cluster-autoscaler in `crusoe-system`, bounds `AUTOSCALER_MIN:MAX:POOL`, image must match the cluster's k8s minor) + `make install-vllm-hpa` (pod tier — KEDA + a lightweight Prometheus scraping `vllm:num_requests_waiting`) + `make deploy-amd-mi355x AUTOSCALE=1` (emits `spec.scaling` min/max instead of a fixed `replicas`, so KServe hands the replica count to the autoscaler). See `manifests/cluster-autoscaler.yaml`, `manifests/prometheus-vllm.yaml`, `manifests/keda-scaledobject.yaml`. Node scale-up to a serving replica measured ~26 min (image pull + AITER compile dominate; node provision ~4.5 min).
