...
- **Autoscaling gotcha (RWX shared disk):** when a new node is autoscaled in, its `fs.csi.crusoe.ai/*` node labels (which the RWX weights PV's `nodeAffinity` requires) land ~4.5 min *after* the node is `Ready` — the fs-node driver works immediately, only the labels lag. During that gap the new replica can't schedule (`PersistentVolume and node mismatch ... no matching NodeSelectorTerms`) **and the cluster-autoscaler over-provisions** (adds a 2nd node for 1 replica), later reclaiming it via scale-down. It self-resolves once the labels apply; to avoid it, pre-apply the labels or raise the autoscaler's scale-up delay.
